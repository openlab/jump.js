...
( function($) {
	...
	$.a('ui.jump', '1.0', {
		...
		_setup: function(options, element, callback) {
			this.el = element;
			this.$el = $(this.el);
			this.options = this._extend({ 'zoom': 5, 'credentials': Microsoft.Maps.Credentials }, options, { 'center': { 'callback': this._latLng } } );
			...
			for ( var key in options ) {
				if ( replace && replace[key] ) {
					...
					if ( typeof replace === 'string' ) {
						...
						defaults[key] = (replace.callback) ? replace.callback(options[key]) : options[key];
					}
				} else {
